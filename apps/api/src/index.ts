...
app.get("/api/transactions", authMiddleware, async (c) => {
  ...
  const rows = await db.select().from(transactions).where(eq(transactions.userId, userId)).orderBy(transactions.createdAt);
...
app.post("/api/withdraw", authMiddleware, async (c) => {
  ...
  const newBalance = await db.transaction(async (tx) => {
    const [updatedBalance] = await tx
      .update(balances)
      .set({
        amount: sql`${balances.amount} - ${String(amount)}`,
        updatedAt: new Date(),
      })
      .where(and(eq(balances.userId, userId), sql`${balances.amount} >= ${String(amount)}`))
    ...
    return Number(updatedBalance.amount);
  });
...
app.post("/api/checkout", authMiddleware, async (c) => {
  ...
  try {
    const [existing] = await db.select().from(transactions).where(eq(transactions.idempotencyKey, body.idempotency_key));
